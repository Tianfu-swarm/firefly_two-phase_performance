...
for N in Ns:
    ...
    for C in Cs:
        ...
        k_path = f'/Volumes/Data/other/2026_firefly_synchronization/N={N}_clock_lnegth={C}_T={T}_flash_proportion={flash_proportion}{noise_str}_k_regular_graph_flash_counts.csv'
        r_path = f'/Volumes/Data/other/2026_firefly_synchronization/N={N}_clock_lnegth={C}_T={T}_flash_proportion={flash_proportion}{noise_str}_r_com_range_flash_counts.csv'
        
        try:
            data = pd.read_csv(k_path)
            ...
            save_flash_counts[N][C] = (full_communication_data.values < (N*0.9)).sum() / full_communication_data.shape[0]
        except:
            print(f"File not found: {k_path}")
            continue
