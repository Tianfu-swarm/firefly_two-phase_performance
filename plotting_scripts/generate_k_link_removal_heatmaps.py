...
heatmap_path_00_async_lower_phase = (f"/home/till/PycharmProjects/firefly_two-phase_performance/results/"
                f"compressed_async_lower_phase_runs_results_k_graph_experiment{experiment_tag}_N={np.min(Ns)}_{np.max(Ns)}_C={np.min(Cs)}_{np.max(Cs)}_T={T}_flash_proportion=0.5_qr_threshold=0.5_update_noise={0.0}.npz")
heatmap_path_005_async_lower_phase = (f"/home/till/PycharmProjects/firefly_two-phase_performance/results/"
                f"compressed_async_lower_phase_runs_results_k_graph_experiment{experiment_tag}_N={np.min(Ns)}_{np.max(Ns)}_C={np.min(Cs)}_{np.max(Cs)}_T={T}_flash_proportion=0.5_qr_threshold=0.5_update_noise={0.05}.npz")
heatmap_path_01_async_lower_phase = (f"/home/till/PycharmProjects/firefly_two-phase_performance/results/"
                f"compressed_async_lower_phase_runs_results_k_graph_experiment{experiment_tag}_N={np.min(Ns)}_{np.max(Ns)}_C={np.min(Cs)}_{np.max(Cs)}_T={T}_flash_proportion=0.5_qr_threshold=0.5_update_noise={0.1}.npz")
heatmap_path_02_async_lower_phase = (f"/home/till/PycharmProjects/firefly_two-phase_performance/results/"
                f"compressed_async_lower_phase_runs_results_k_graph_experiment{experiment_tag}_N={np.min(Ns)}_{np.max(Ns)}_C={np.min(Cs)}_{np.max(Cs)}_T={T}_flash_proportion=0.5_qr_threshold=0.5_update_noise={0.2}.npz")
...
save_flash_counts_00_async_lower_phase = {}
save_flash_counts_005_async_lower_phase = {}
save_flash_counts_01_async_lower_phase = {}
save_flash_counts_02_async_lower_phase = {}
for N in Ns:
    ...
    save_flash_counts_00_async_lower_phase[N] = {}
    save_flash_counts_005_async_lower_phase[N] = {}
    save_flash_counts_01_async_lower_phase[N] = {}
    save_flash_counts_02_async_lower_phase[N] = {}
    for C in Cs:
        ...
        save_flash_counts_00_async_lower_phase[N][C] = 0.0
        save_flash_counts_005_async_lower_phase[N][C] = 0.0
        save_flash_counts_01_async_lower_phase[N][C] = 0.0
        save_flash_counts_02_async_lower_phase[N][C] = 0.0
        ...
        try:
            ...
            # lower phase
            for run in data[int(N)].keys():
                if np.max(data[int(N)][run]) <= N * 0.85:
                    save_flash_counts_00_async_lower_phase[N][C] += 1
                if np.max(data[int(N - (N * 0.05))][run]) <= N * 0.85:
                    save_flash_counts_005_async_lower_phase[N][C] += 1
                if np.max(data[int(N - (N * 0.1))][run]) <= N * 0.85:
                    save_flash_counts_01_async_lower_phase[N][C] += 1
                if np.max(data[int(N - (N * 0.2))][run]) <= N * 0.85:
                    save_flash_counts_02_async_lower_phase[N][C] += 1
            ...
            save_flash_counts_00_async_lower_phase[N][C] = np.nan
            save_flash_counts_005_async_lower_phase[N][C] = np.nan
            save_flash_counts_01_async_lower_phase[N][C] = np.nan
            save_flash_counts_02_async_lower_phase[N][C] = np.nan
...
heatmap_00_async_lower_phase = np.zeros((len(Cs), len(Ns)))
heatmap_005_async_lower_phase = np.zeros((len(Cs), len(Ns)))
heatmap_01_async_lower_phase = np.zeros((len(Cs), len(Ns)))
heatmap_02_async_lower_phase = np.zeros((len(Cs), len(Ns)))
...
for i, C in enumerate(Cs):
    for j, N in enumerate(Ns):
        ...
        heatmap_00_async_lower_phase[i, j] = save_flash_counts_00_async_lower_phase.get(N, {}).get(C, np.nan)
        heatmap_005_async_lower_phase[i, j] = save_flash_counts_005_async_lower_phase.get(N, {}).get(C, np.nan)
        heatmap_01_async_lower_phase[i, j] = save_flash_counts_01_async_lower_phase.get(N, {}).get(C, np.nan)
        heatmap_02_async_lower_phase[i, j] = save_flash_counts_02_async_lower_phase.get(N, {}).get(C, np.nan)
...
np.savez_compressed(heatmap_path_00_async_lower_phase, arr=heatmap_00_async_lower_phase)
np.savez_compressed(heatmap_path_005_async_lower_phase, arr=heatmap_005_async_lower_phase)
np.savez_compressed(heatmap_path_01_async_lower_phase, arr=heatmap_01_async_lower_phase)
np.savez_compressed(heatmap_path_02_async_lower_phase, arr=heatmap_02_async_lower_phase)
